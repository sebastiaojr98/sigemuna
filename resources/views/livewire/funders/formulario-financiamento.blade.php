<form class="row" wire:submit="createFinancing()">
    ...
    <div class="col-7 form-group">
        <label for=""><span class="span-required">*</span>Financiador</label>
        <select name="" id="" class="form form-control" wire:model="financier">
            <option value="" selected>escolha...</option>
            @foreach($financiers as $financier)
              <option value="{{$financier->id}}">{{$financier->name}}</option>
            @endforeach
        </select>
        @error('financier') <small class="alert-error"> {{ $message }} </small> @enderror
    </div>
    ...
    <div class="col-7 form-group">
        <label for=""><span class="span-required">*</span>Taxa de Juro (%)</label>
