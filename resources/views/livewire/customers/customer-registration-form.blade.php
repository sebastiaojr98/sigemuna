...
<div class="row" wire:submit="createCustomer()">
    ...
    @if ($step === 1)
        <div class="col-5 form-group">
            <label for=""><span class="span-required">*</span>Tipo de Cliente</label>
            <select name="" id="" class="form form-control" wire:model="typeCustomer">
                <option value="" selected>escolha...</option>
                @foreach ($typeCustomers as $typeC)
                    <option value="{{$typeC['name']}}">{{$typeC['description']}}</option>
                @endforeach
            </select>
            @error('typeCustomer') <small class="alert-error"> {{ $message }} </small> @enderror
        </div>
    ...
    @if ($step === 3)
        ...
        <div class="col-6 form-group">
            <label for="">Rua/Avenida (Opcional)</label>
            <input type="text" class="form form-control" placeholder="Rua/Avenida" wire:model="street">
            @error('street') <small class="alert-error"> {{ $message }} </small> @enderror
        </div>

        <div class="col-6 form-group">
            <label for="">Número (Opcional)</label>
            <input type="text" class="form form-control" placeholder="Número" wire:model="streetNumber">
            @error('streetNumber') <small class="alert-error"> {{ $message }} </small> @enderror
        </div>
        <div class="col-12 form-group">
            <label for="">Referência (Opcional)</label>
            <input type="text" class="form form-control" placeholder="Referência" wire:model="reference">
            @error('reference') <small class="alert-error"> {{ $message }} </small> @enderror
        </div>
        <hr class="my-3">
        <div class="col-12 d-md-flex justify-content-md-end">
            <button class="btn btn-secondary px-5 mx-2" wire:click="prevStep()">Anterior</button>
            <button class="btn btn-success px-5" wire:click="createCustomer()">Finalizar</button>
        </div>
    @endif
