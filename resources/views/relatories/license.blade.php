            ...
            <div class="div-principal">
                <div style="background-color: #ffffffde; color: #000000e0; width: 100%; height: 100%;">
                    ...
                    <div style="width: 100%; float: leftx; margin-top: 250px;">
                        <h1 class="nome-licenca">{{mb_strtoupper($license->serviceContracted->service->name, 'UTF-8')}}</h1>
                        <h1 style="margin-top: 30px; font-weight: bold;">Licença N<sup>o</sup> {{$license->code}} / EMUSANA / {{ date("d/m/Y", strtotime($license->issue_date)) }}</h1>

                        @if ($license->car_brand)
                            <p style="text-align: justify; line-height: 25px; font-weight: 400;">Nos termos de regulamento do licenciamento de abastecimento e transporte de água potável por fornecedores privados, é concedida a licença de transporte de água potável a <strong>{{ $license->customer->name }}</strong> com o número Único de Identificação Tributaria (NUIT) <strong>{{$license->customer->nuit}}</strong>, proprietária da viatura de marca <strong>{{$license->car_brand}}</strong>, modelo <strong>{{$license->car_model}}</strong>, matricula <strong>{{$license->car_registration}}</strong>, com práticas de atividade no Distrito Municipal de Nampula, de acordo com as condições gerais fixadas no presente documento.</p>
                        @else
                            <p style="text-align: justify; line-height: 25px; font-weight: 400;">Nos termos de regulamento do licenciamento de abastecimento e transporte de água potável por fornecedores privados, é concedida a licença de abastecimento de água potável a <strong>{{ $license->customer->name }}</strong> com o número Único de Identificação Tributaria (NUIT) <strong>{{$license->customer->nuit}}</strong>, localizada no Bairro de <strong>{{$license->communalUnit->neighborhood->label}}</strong>, Unidade Comunal <strong>{{$license->communalUnit->label}}</strong>, Quarteirão <strong>N<sup>o</sup> {{$license->block ? $license->block : "-"}}</strong>, Casa <strong>N<sup>o</sup> {{$license->house_number ? $license->house_number : "-"}}</strong>, com práticas de atividade no Distrito Municipal de Nampula, de acordo com as condições gerais fixadas no presente documento.</p>
                        @endif
